...
html+='<div class="rssHeader">'+'<a href="'+feeds.link+'" title="'+feeds.description+'">'+feeds.title+'</a>'+'</div>';html+='<div class="rssBody">'+'<ul>';for(var i=0;i<feeds.entries.length;i++){var entry=feeds.entries[i];var entryDate=new Date(entry.publishedDate);/*var pubDate=entryDate.toLocaleDateString()+' '+entryDate.toLocaleTimeString();*/var pubDate=pad(entryDate.getDate(), 2) + "-" + pad(entryDate.getMonth()+1, 2) + "-" + entryDate.getFullYear() + " a les " + pad(entryDate.getHours(), 2) + ":" + pad(entryDate.getMinutes(), 2);html+='<li class="rssRow '+row+'">'+'<'+options.titletag+'><a href="'+entry.link+'" title="View this feed at '+feeds.title+'">'+entry.title+'</a></'+options.titletag+'>'
...
html+='</ul>'+'</div>'
feed=$(html);
feed.find('embed').attr('width', 425).attr('height', 344);
feed.find('img').attr('width', 425).attr('height', 344);
$(e).html(feed.html());};})(jQuery);